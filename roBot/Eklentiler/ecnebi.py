...
@Client.on_message(filters.command(['ingilizce'], ['!','.','/']))
async def ingilizce(client, message):
    ...
    if cevirici.detect(girdi).lang != 'en':
        await ilk_mesaj.edit("__Veri ingilizce olmadığı için türkçeye çeviremiyorum..__")
        return
...
@Client.on_message(filters.command(['türkçe'], ['!','.','/']))
async def türkçe(client, message):
    ...
    if cevirici.detect(girdi).lang != 'tr':
        await ilk_mesaj.edit("__Veri türkçe olmadığı için ingilizceye çeviremiyorum..__")
        return
